"""
Logging Agent Chat — Lightweight streaming chat for the Dashboard server
─────────────────────────────────────────────────────────────────────────
Talks to the *logging-agent* hosted in Azure AI Foundry, dispatches the
four analytics tools locally (list_agents, list_all_students,
...
keeping only the pieces relevant to the logging/analytics agent.
"""
...
def _get_openai_client():
    ...
    if _openai_client is None:
        ...
        logger.info("AIProjectClient + OpenAI client initialised for logging-agent")
...
def chat_stream(
    ...
    """
    Send a message to the logging-agent and yield SSE-style events.
            ...
            if etype == "response.output_text.delta":
                ...
                if item and getattr(item, "type", None) == "function_call":
                    ...
                    if func_name == "add_message":
                        ...
                        try:
                            args = json.loads(func_args) if isinstance(func_args, str) else func_args
                            result_str = tools.TOOL_HANDLERS[func_name](args)
