...
import { Copy, Check, Info, Lightbulb, MessageSquareWarning, TriangleAlert, OctagonAlert } from "lucide-react";
import "katex/dist/katex.min.css";

/* ----------------------------- GitHub alerts ----------------------------- */

const ALERT_STYLES = {
  NOTE: { label: "Note", icon: Info, accent: "border-sky-500/70", tint: "bg-sky-500/[0.07]", text: "text-sky-300" },
  TIP: { label: "Tip", icon: Lightbulb, accent: "border-emerald-500/70", tint: "bg-emerald-500/[0.07]", text: "text-emerald-300" },
  IMPORTANT: { label: "Important", icon: MessageSquareWarning, accent: "border-violet-500/70", tint: "bg-violet-500/[0.07]", text: "text-violet-300" },
  WARNING: { label: "Warning", icon: TriangleAlert, accent: "border-amber-500/70", tint: "bg-amber-500/[0.07]", text: "text-amber-300" },
  CAUTION: { label: "Caution", icon: OctagonAlert, accent: "border-rose-500/70", tint: "bg-rose-500/[0.07]", text: "text-rose-300" },
} as const;

type AlertKind = keyof typeof ALERT_STYLES;

/** GitHub writes alerts as a blockquote whose first line is `[!NOTE]`; peel that marker off. */
function extractAlert(children: React.ReactNode): { kind: AlertKind; body: React.ReactNode[] } | null {
  const items = React.Children.toArray(children);
  for (let index = 0; index < items.length; index += 1) {
    const element = items[index];
    if (!React.isValidElement(element)) continue;
    const inner = React.Children.toArray((element.props as { children?: React.ReactNode }).children);
    const first = inner[0];
    if (typeof first !== "string") return null;
    const match = first.match(/^\s*\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*\n?/i);
    if (!match) return null;
    const remainder = first.slice(match[0].length);
    const body = [...items];
    body[index] = React.cloneElement(
      element as React.ReactElement<{ children?: React.ReactNode }>,
      {},
      remainder ? [remainder, ...inner.slice(1)] : inner.slice(1),
    );
    return { kind: match[1].toUpperCase() as AlertKind, body };
  }
  return null;
}
...
// Loaded on demand so the (large) mermaid bundle never lands in the initial chunk.
  ...
  if (!mermaidReady) {
    mermaidReady = import("mermaid").then(({ default: mermaid }) => {
      mermaid.initialize({
        startOnLoad: false,
        theme: "dark",
        // Diagram source is model-generated, so keep HTML labels and click handlers off.
...
function MermaidBlock({ code }: { code: string }) {
  ...
  if (failed) {
    return (
      <pre className="my-4 overflow-x-auto rounded-xl bg-[#1e1e1e] px-5 py-4 text-[13px] font-mono text-neutral-300">
        {code}
      </pre>
    );
  }
  ...
  if (!svg) {
    return (
      <div className="my-4 rounded-xl border border-neutral-700/60 bg-neutral-900/40 px-5 py-6 text-sm text-neutral-500">
        Rendering diagram…
      </div>
    );
  }
  ...
  return (
    <div
      className="my-4 overflow-x-auto rounded-xl border border-neutral-700/60 bg-neutral-900/40 px-4 py-4 [&_svg]:mx-auto [&_svg]:h-auto [&_svg]:max-w-full"
      // Mermaid renders in strict mode, which escapes label markup.
...
function preprocessCitations(content: string, sources?: SourceItem[]): string {
  ...
  return cleanContent.replace(citationPattern, (match, mainNum, subNum, text) => {
...
const sanitizeSchema = {
  ...
  tagNames: [
    ...
    // Inline SVG figures
    'circle', 'ellipse', 'polyline', 'polygon', 'text', 'tspan', 'defs', 'marker', 'title',
    // Rich prose: collapsibles, definition lists, typographic tags, footnotes
    'details', 'summary', 'dl', 'dt', 'dd', 'kbd', 'mark', 'abbr', 'sub', 'sup',
    'ins', 'del', 's', 'section', 'input', 'br', 'hr', 'caption', 'colgroup', 'col', 'tfoot',
    // Image elements
  ...
  attributes: {
    ...
    // Merged cells arrive as raw HTML tables.
    th: [...(defaultSchema.attributes?.th || []), 'colSpan', 'rowSpan', 'colspan', 'rowspan', 'align', 'scope', 'class'],
    td: [...(defaultSchema.attributes?.td || []), 'colSpan', 'rowSpan', 'colspan', 'rowspan', 'align', 'class'],
    sup: ['class', 'title'],
    figure: ['class'],
    figcaption: ['class'],
    ...
    text: ['x', 'y', 'dx', 'dy', 'font-size', 'font-weight', 'font-style', 'text-anchor', 'fill', 'class'],
...
const components: Components = {
  ...
  blockquote({ children }) {
    ...
    if (alert) {
      const { label, icon: AlertIcon, accent, tint, text } = ALERT_STYLES[alert.kind];
      return (
        <div className={`my-4 rounded-r-lg border-l-4 ${accent} ${tint} px-5 py-3`}>
          <p className={`mb-1 flex items-center gap-2 text-sm font-semibold ${text}`}>
            <AlertIcon className="h-4 w-4" aria-hidden="true" />
            {label}
          </p>
          <div className="text-neutral-100 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">
            {alert.body}
          </div>
        </div>
      );
  ...
  pre(props) {
    ...
    const language = React.isValidElement(child)
      ? ((child.props as { className?: string }).className || "").match(/language-(\w+)/)?.[1]
      : undefined;
    if (language === "mermaid") {
      return <MermaidBlock code={codeTextOf(child).replace(/\n$/, "")} />;
    }
