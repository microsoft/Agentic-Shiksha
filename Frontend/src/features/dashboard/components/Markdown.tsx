...
/**
 * Turn the insights agent's inline evidence markers, written as [[S1-TP1]],
 * into anchors that point at the matching chip in the References panel.
 */
function preprocessEvidenceRefs(content: string): string {
  return content.replace(
    /\[\[\s*(S\d+-(CI|TC|TP|AS)\d+)\s*\]\]/g,
    (_match, ref: string, kind: string) =>
      `<a href="#evidence-${ref}" class="evidence-ref evidence-kind-${kind}" title="Jump to reference ${ref}">${ref}</a>`,
  );
}
...
const components: Components = {
  ...
  a({ href, children, className, title, ...props }) {
    // Inline evidence marker — the surrounding bubble handles the jump.
    if (className?.includes('evidence-ref')) {
      // Match the reference-chip palette: inventories, concepts, topics, assets.
      const kind = className.match(/evidence-kind-(CI|TC|TP|AS)/)?.[1] ?? '';
      const palette: Record<string, string> = {
        CI: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300 hover:bg-emerald-500/20',
        TC: 'border-violet-500/30 bg-violet-500/10 text-violet-300 hover:bg-violet-500/20',
        TP: 'border-sky-500/30 bg-sky-500/10 text-sky-300 hover:bg-sky-500/20',
        AS: 'border-amber-500/30 bg-amber-500/10 text-amber-300 hover:bg-amber-500/20',
      };
      return (
        <a
          href={href}
          title={title}
          className={`evidence-ref evidence-kind-${kind} inline-flex items-center rounded-md border px-1.5 py-px mx-0.5 align-baseline text-[11px] font-medium tabular-nums no-underline transition-colors cursor-pointer ${
            palette[kind] ?? 'border-neutral-700 bg-neutral-800 text-neutral-300 hover:bg-neutral-700'
          }`}
          {...props}
        >
          {children}
...
export default function Markdown({
  ...
  const processedContent = preprocessEvidenceRefs(preprocessCitations(mathProcessed, sources));
  ...
  return (
    <div className={`max-w-none ${className || ""}`}>
      ...
      <ReactMarkdown
        ...
        rehypePlugins={[
          rehypeRaw,
          // Skip sanitize when math is enabled - KaTeX output needs full HTML support
          // Our content comes from our own API so sanitization is less critical
          ...(!math ? [[rehypeSanitize, sanitizeSchema]] : []),
