...
/** `<prefix>-<timestamp>-<random>`, for ids that are also read by humans in logs. */
export function prefixedId(prefix: string, bytes = 6): string {
  return `${prefix}-${Date.now().toString(36)}-${randomToken(bytes)}`;
}
