...
which include [Microsoft](https://github.com/microsoft),
...
information can be found at [microsoft.com/msrc](https://www.microsoft.com/msrc).
...
* **Never commit secrets.** All `.env` files are git-ignored, and `Backend/.dockerignore`
  ...
  precedence over `.env`.
* **Prefer managed identity over keys.** Cosmos DB, Azure AI Search, Blob Storage and
  Microsoft Foundry are all accessed with Entra ID tokens in the default configuration.
* **Rotate `JWT_SECRET` per environment.** It signs session cookies; generate a unique
  high-entropy value with
  `python -c "import secrets; print(secrets.token_urlsafe(64))"`.
* **Values prefixed `VITE_` are public.** Vite inlines them into the browser bundle at
  build time. Never place a credential in one.
